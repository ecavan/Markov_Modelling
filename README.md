...
In conclusion, our project has reproduced the result from Shulte & al. to compare NFL receivers fairly by comparing them to other members in their cluster. To do this, we reproduced a Markov model that emulates the non-shot expected goals models found in soccer and hockey. In this way, we reproduce the 'EPA' (expected points added metric developped by [Virgil Carter](https://www.thescore.com/nfl/news/2193857)). Although the two metrics are scaled differently, it is interesting to see the difference in how we rank players relative to this mainstream metric. We also reproduced the k-means algorithm, but we adpated it to try to deal with some of the shortcomings of the algorithm. Specifically, we were able to choose an optimal value of k (the number of clusters) and by calculating a player's affinity to each of the k clusters we can reassign players and end up with unequal cluster sizes. Throughout this project we learned more about how to evaluate results from clustering and how to deal with large amounts of data (12 million rows) to implement the theoretical markov chain in a practical setting of Sprts Analytics.
